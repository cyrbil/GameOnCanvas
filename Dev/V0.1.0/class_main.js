function game(p1,p2,p3,p4){
    ...
    // constructor
    game.prototype.construct = function(obj){
        console.log("Params to parse:",obj);
    }
    
    
    // default params
    game.prototype.canvas_rect  = new rectangle(0,0,600,400); // dimensions of canvas
    game.prototype.antialiasing = 1;                          // no antialiasing
    
    // parse params and update default params, yeah that's a lot of various parameters ...
    if(typeof(p1) == "rectangle"){
        this.canvas_rect = p1;
        ...
        if((typeof(p3) == "number" || typeof(p3) == "string") && p3 > 1)
    ...
    game.prototype.div          = null; // div that contain game data + canvas
    game.prototype.canvas       = null; // LE canvas
    game.prototype.ctx          = null; // context of canvas
    game.prototype.fps          = 30;   // number of frame per seconds targeted
    game.prototype.ressources   = ({fake:"test"}); // object of string to load before launch main loop (preload) 
    game.prototype.canvas_inner = "Your web browser is to old to let you play. Please update or change it."; // between <canvas> balises
    game.prototype.scene        = new scene(); // scene that will be printed one game launch
    game.prototype.debug        = true; // display debug corner
    game.prototype.keyboard = {shift:false,ctrl:false,alt:false,altG:false,code:null,ascii:null};
    game.prototype.mouse    = {left:false,middle:false,right:false,x:0,y:0};
    game.prototype.debug_fps = 0;
    game.prototype.array_fps = [];
    ...
    game.prototype.init = function(){
        ...
        this.ctx.fillText("V0.37 Alpha", 15, 55);
    ...
    game.prototype.load = function(timeout){
        ...
        for (var i in this.ressources) {
            if(typeof(this.ressources[i]) != "string"){ // skipping if not a string
                ...
                // load ressource
                object[i] = new elem(this.ressources[i]);
                object_size++;
    ...
    game.prototype.ticker = function(tick, date, frames){
		if(tick==null||date==null||frames==null) // initial launch
			return this.ticker(new Date().getTime(),new Date().getTime(),0);
		...
		// else, tick
		return this.ticker((new Date().getTime()), date, frames);
    }
    
    // reset or format events
    game.prototype.events = function(e,that){ // if no e, reset event
    ...
    game.prototype.display = function(){
        ...
        // call move() for each element in current scene
        this.scene.tick();
        ...
        if(this.debug){
            ...
            this.ctx.fillText(
                ...
                ",KA-"+((this.keyboard.alt)?"T":"F")+
                ",KC-"+((this.keyboard.ctrl)?"T":"F")+
...
function rectangle(x,y,w,h){ // simple object to manage coord
